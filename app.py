...
with col_right:
    ...
    carbon_tons = area * 12.5  # Example calculation

    if st.button("Generate ESG Report"):
        # pdf_data is created HERE
        pdf_data = create_pdf_report(farm_name, area, carbon_tons)

        st.success("✅ Report Generated Successfully!")

    st.download_button(
        label="📄 Download ESG Report",
        data=bytes(pdf_data),  # Force conversion to bytes
        file_name="EcoPlot_Report.pdf",
        mime="application/pdf"  # Tell the browser this is a PDF
    )
